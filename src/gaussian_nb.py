...
class GaussianNBWeighting(WeightingModel):
    """
    ...
    def __init__(self, features: List[str], ppin: str):
        super().__init__(features, ppin, GaussianNB())
...
if __name__ == "__main__":
    ...
    weighting = GaussianNBWeighting(features, ppin)
    ...
    if df_w_ppin is not None:
        ...
        df_w_ppin.write_csv(
            f"../data/weighted/{ppin}_weighted_gnb.csv",
            has_header=False,
            separator="\t",
        )
        df_w_ppin_20k.write_csv(
            f"../data/weighted/{ppin}_weighted_gnb_20k.csv",
            has_header=False,
            separator="\t",
        )
