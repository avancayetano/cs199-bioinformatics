...
class ClustersEvaluator:
    ...
    def evaluate_complex_prediction(self):
        ...
        df_evals.write_csv(
            f"../data/evals/test_{prefix}cluster_evals.csv", has_header=True
        )
    ...
    def get_complex_prediction_metrics(
        ...
        prec_numerator = len(
            list(
                filter(
                    lambda cluster: (
                        self.there_is_match(cluster, test_complexes, match_thresh)
                    ),
                    clusters,
                )
            )
    ...
    def prec_recall_curves(self):
        ...
        """
        prefix = "dip_" if self.dip else ""
        df_cluster_evals = pl.read_csv(f"../data/evals/test_{prefix}cluster_evals.csv")
